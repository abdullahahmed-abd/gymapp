      ...
      <View style={styles.membershipHeader}>
        <View style={{ flex: 1 }}>
          ...
          >
            ...
            />
            <Text
              style={[styles.workoutBadgeText, { color: Colors.zinc[200] }]}
            >
              LIMITED FEATURES
            </Text>
          </View>
...
const RequestPendingCard = ({ request }) => {
  ...
  return (
    <View style={[styles.membershipCard, { borderColor: `${accentColor}35` }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {request?.workoutType && (
            ...
            >
              <HugeiconsIcon
                icon={
                  request.workoutType === 'cardio_weights'
                    ? Activity01Icon
                    : Dumbbell01Icon
                }
                size={ms(10)}
      ...
      <View
        style={[styles.cardDivider, { backgroundColor: `${accentColor}20` }]}
      />
  ...
  return (
    <View
      style={[
        ...
        { borderColor: `${template.iconColor}40` },
      ]}
    >
      <View style={styles.allmemberLogo}>
        <Image source={gymlogoimg} style={styles.allmemberLogoimg} />
      ...
      <View style={styles.membershipHeader}>
        <View style={{ flex: 1 }}>
          <View style={styles.liveBadge}>
            ...
            />
            <Text
              style={[styles.liveBadgeText, { color: template.iconColor }]}
            >
              Active Plan
            </Text>
          </View>
            ...
            >
              ...
              >
                {membershipData.workoutType === 'cardio_weights'
                  ...
                  : membershipData.workoutType === 'weights_only'
                  ? 'WEIGHT LIFTING ONLY'
                  : 'CARDIO ONLY'}
              </Text>
            </View>
      ...
      <View
        style={[
          styles.cardDivider,
          { backgroundColor: `${template.iconColor}25` },
        ]}
      />
      <View style={styles.membershipFooter}>
...
const PlanCard = ({ plan, onSelect, submitting }) => {
  ...
  return (
    ...
    >
      <View
        style={[styles.planCard, { borderColor: `${template.iconColor}40` }]}
      >
        <LinearGradient
          colors={[
            `${template.iconColor}12`,
            `${template.iconColor}06`,
            'transparent',
          ]}
        ...
        <View style={styles.planCardHeader}>
          <View style={{ flex: 1, marginRight: s(12) }}>
            ...
            >
              ...
              />
              <Text
                style={[styles.planBadgeText, { color: template.iconColor }]}
              >
        ...
        {plan.hasOffer && plan.offer && plan.offer.text ? (
...
const UserDashboardScreen = ({ navigation }) => {
  ...
  // ✅ Replace with useAuth() in production
  const currentUser = {
    id: 'user_001',
    name: 'Abdullah Ahmed',
  ...
  useEffect(() => {
    if (showPlansModal) {
      refreshPlans()
        .then(() => safeLoadPlans())
        .catch((e) => {
          console.error('refreshPlans error:', e);
          setActivePlans([]);
        });
    }
  }, [showPlansModal, safeLoadPlans, refreshPlans]);

  // ✅ Handle openPlans param from other screens (e.g. MembersFriendScreen)
  useEffect(() => {
  ...
  const handleSelectPlan = async (plan) => {
    ...
    Alert.alert(
      'Confirm Request',
      `Do you want to request "${plan.name}" membership for $${price}/${duration}?`,
      [
    ...
    >
      ...
      >
        <SafeAreaView style={styles.safeArea} edges={['top']}>
              ...
              {isPendingApproval ? (
                ...
                <View
                  style={[
                    ...
                    {
                      backgroundColor: `${
                        approvedMember?.template?.iconColor || Colors.gold
                      }15`,
                      borderColor: `${
                        approvedMember?.template?.iconColor || Colors.gold
                      }30`,
                ...
                >
                  <HugeiconsIcon
                    icon={Medal02Icon}
                    size={ms(10)}
                    color={approvedMember?.template?.iconColor || Colors.gold}
                  />
                  <Text
                    style={[
                      styles.userTypeBadgeText,
                      {
                        color:
                          approvedMember?.template?.textColor || Colors.gold,
                      },
                    ]}
                  >
              ...
              <EliteMembershipCard
                membershipData={{
                  ...
                  expiryDate: approvedMember?.expiryDate
                    ? new Date(approvedMember.expiryDate).toLocaleDateString(
                        'en-US',
                        {
                          day: 'numeric',
                          month: 'short',
          ...
          {/* ✅ Correct route names from AppNavigator */}
         <BottomNav
  activeTab="home"
  onTabChange={(tab) => {
    if (tab === 'home') {}
